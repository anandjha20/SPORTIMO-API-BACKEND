...
function ReportListTable(props) {
        ...
        e.preventDefault();
         ...
         setFromvalue(formData);
           console.log('Formvlaues === ', Formvlaues);
            axios.post(`/web_api/reported_user_list`, formData, options)
           .then(res => {
    ...
    const userlist = async () => {
        await axios.post(`/web_api/reported_user_list` , options)
            .then(res => {
                const data = res.data.body;
                const total = res.data.rows;
                const totalPage = (Math.ceil(total / limit));
                        ...
                        <table className="table ">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col" >Last Report Date</th>
                                ...
                                {data.map((item) => {
                                    return (
                                        <tr key={item._id}>
                                            ...
                                            <td><IconButton  onClick={(e) => { viewFun(item._id); }}><span >{item.total}</span></IconButton></td>
