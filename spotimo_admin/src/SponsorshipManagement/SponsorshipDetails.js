...
function SponsorshipDetails() {
    ...
    const get_data = async(id)=>{
      ...
      {  try{ 
          let formData = {};    let options1 = { headers:{"Content-type": "application/json", "token": localStorage.getItem('token')  }};
          let response = await axios.get('/sponsor_detail/'+id,formData,options1);
