    ...
    try {
      ...
      let tips = tips_trick.find({}, function (err, tips) {
        if (err) {
          ...
          tips.map((item) => {
            if (language == 'ar') { item.tips_trick = item.tips_trick_ara }
            return item;
          });
    ...
    try {
      ...
      faq_tbl.findOneAndUpdate({ _id: id }, { $set: { question_ara, answer_ara, question, answer, faq_cat_id: cat_id } }, { new: true }, (err, updateddata) => {
        ...
        if (isEmpty(updateddata)) {
          ...
          return res.status(200).json({ "status": true, "msg": "success", "body": updateddata });
        }
      });
    ...
    try {
      ...
      content_tbls.findOneAndUpdate({ type }, { $set: { type, content_data, content_data_ara, content_data_fr } }, { new: true }, (err, updatedUser) => {
        ...
        if (isEmpty(updatedUser)) {
          ...
          add.save((err, datas) => {
            if (err) {
              console.log(err); return res.status(200).send({ "status": false, "msg": 'some errors...' });
            } else {
