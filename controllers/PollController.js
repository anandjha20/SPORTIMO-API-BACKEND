...
class PollController {
  ...
  static poll_data_by_poll_id = async (req, res) => {
    try {

      let _id = req.params.id;
      if (!isEmpty(_id)) { 
      let data = await poll_tbl.findById(_id)
      let sports=data.sports;
      let leagues=data.leagues;
      let teams=data.teams;
      let sports_data=await sport_list.find({_id:{$in:sports}}).select('name');
      let sportArr=[]
      sports_data.map((item)=>{
        sportArr.push({value:item._id.toString(),label:item.name})
      })
      let leagues_data=await league_list.find({season_id:{$in:leagues}}).select('season_id original_name');
      let leagueArr=[]
      leagues_data.map((item)=>{
        leagueArr.push({value:item.season_id,label:item.original_name})
      })
      let teams_data=await team_list.find({team_id:{$in:teams}}).select('team_id team_name');
      let teanArr=[]
      teams_data.map((item)=>{
        teanArr.push({value:item.team_id,label:item.team_name})
      })
      data.sports=sportArr;
      data.leagues=leagueArr;
      data.teams=teanArr;
      console.log({sportArr,leagueArr,teanArr})
      return res.status(200).send({ 'status': true, 'msg':  "success",  'body': data });
    }else{
      return res.status(200).send({ 'status': true, 'msg': "something went wrong",  'body': data });
  ...
  static poll_list_user = async (req, res) => {
    try {
      ...
      add.save((err, data) => {
        if (err) {
          ...
          if (user_data.noti_status == 1) {
            ...
            let poll_noti_parms = {
              "sports": user_data.sports, "leagues": user_data.leagues, "teams": user_data.teams,
